...
from qt.core import (
    ...
    QFont, QFrame, QGroupBox, QHBoxLayout, QHeaderView, QLabel,
    QLineEdit, QListWidget, QListWidgetItem, QProgressBar, QPushButton,
    QScrollArea, QSize, QSplitter, QTableWidget, QTableWidgetItem,
    QTabWidget, QTextEdit, Qt, QVBoxLayout, QWidget,
)


class AnalysisDialog(QDialog):
    def __init__(self, parent, title, author, fmt, text, character_stats):
        ...
        self.character_hsk = hsk_coverage(character_stats["unique_chars"])
    ...
    def _on_mode_changed(self, index):
        ...
        if mode == self.mode:
            return
        if mode == "word" and not self._ensure_word_stats():
    ...
    def _ensure_word_stats(self):
        if self.word_stats is not None:
            return True

        from calibre_plugins.chinese_character_analyzer.analyzer import (
            SegmentationError, analyze_words,
        )
        ...
        try:
            ...
            self.word_stats = analyze_words(self.book_text)
            return True
    ...
    def _current_hsk(self):
        if self.mode == "word":
            return self.word_stats["hsk_word_coverage"]
    ...
    def _build_stats_panel(self):
        ...
        if self.mode == "word":
            avg_len = 0.0
            if self._current_total_count():
                weighted_total = sum(len(word) * count for word, count in stats["frequency"].items())
                avg_len = weighted_total / self._current_total_count()
    ...
    def _build_frequency_tab(self):
        ...
        if self.mode == "word":
            ...
            self.table.setHorizontalHeaderLabels(["Rank", "Word", "Count", "Length", "HSK Level"])
            ...
            self.table.setHorizontalHeaderLabels(["Rank", "Character", "Count", "HSK Level"])
            self.table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
            self.table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
            self.table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
            self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)
        ...
        self._populate_table(stats["frequency"])
        layout.addWidget(self.table, stretch=1)

        count = stats["unique_count"]
        tab_label = f"Frequency ({count:,} words)" if self.mode == "word" else f"Frequency ({count:,} chars)"
        self.tabs.addTab(widget, tab_label)

    def _build_hsk_detail_tab(self):
        from calibre_plugins.chinese_character_analyzer.hsk_data import (
            HSK_LEVEL_ORDER, HSK_WORD_LEVEL_ORDER,
        )
        ...
        coverage = self._current_hsk()
        level_order = HSK_WORD_LEVEL_ORDER if self.mode == "word" else HSK_LEVEL_ORDER
        key = "words" if self.mode == "word" else "chars"
        ...
        self.tabs.addTab(widget, "HSK Level Detail")
    ...
    def _build_anki_tab(self):
        ...
        total_known_occ = sum(frequency.get(item, 0) for item in known_in_book)
        ...
        table = QTableWidget()
        unknown_sorted = sorted(unknown_in_book, key=lambda item: frequency.get(item, 0), reverse=True)
        if self.mode == "word":
            table.setColumnCount(5)
            table.setHorizontalHeaderLabels(["#", "Word", "Occurrences", "Length", "HSK Level"])
            ...
            table.setHorizontalHeaderLabels(["#", "Character", "Occurrences", "HSK Level"])
            table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
            table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
            table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
            table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)
        ...
        table.setRowCount(len(unknown_sorted))

        for row, item in enumerate(unknown_sorted):
            idx_item = QTableWidgetItem(str(row + 1))
            idx_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            table.setItem(row, 0, idx_item)

            unit_item = QTableWidgetItem(item)
            unit_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            unit_item.setFont(QFont("Microsoft YaHei", 14 if self.mode == "word" else 16))
            table.setItem(row, 1, unit_item)

            occ_item = QTableWidgetItem(f"{frequency.get(item, 0):,}")
            occ_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            table.setItem(row, 2, occ_item)

            if self.mode == "word":
                length_item = QTableWidgetItem(str(len(item)))
                length_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                table.setItem(row, 3, length_item)

                hsk_item = QTableWidgetItem(self._display_hsk_level(item))
                hsk_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                hsk_item.setFont(QFont("Microsoft YaHei", 9))
                table.setItem(row, 4, hsk_item)
            else:
                hsk_item = QTableWidgetItem(self._display_hsk_level(item))
                hsk_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                hsk_item.setFont(QFont("Microsoft YaHei", 9))
                table.setItem(row, 3, hsk_item)

        layout.addWidget(table)

        row_height = table.verticalHeader().defaultSectionSize()
        header_height = table.horizontalHeader().height() if table.horizontalHeader().height() > 0 else 30
        table_height = header_height + row_height * len(unknown_sorted) + 4
        ...
        self.tabs.addTab(scroll, f"Anki ({coverage_pct:.0f}% coverage)")
    ...
    def _hsk_level_for_item(self, item):
        if self.mode == "word":
            from calibre_plugins.chinese_character_analyzer.hsk_data import HSK_WORD_TO_LEVEL
            return HSK_WORD_TO_LEVEL.get(item, "—")

        from calibre_plugins.chinese_character_analyzer.hsk_data import HSK_LEVELS, HSK_LEVEL_ORDER
        for level in HSK_LEVEL_ORDER:
            if item in HSK_LEVELS[level]:
                return level
        return "—"
    ...
    def _populate_table(self, frequency):
        self.table.setRowCount(len(frequency))
        for row, (item, count) in enumerate(frequency.items()):
            rank_item = QTableWidgetItem(str(row + 1))
            rank_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.table.setItem(row, 0, rank_item)

            value_item = QTableWidgetItem(item)
            value_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            value_item.setFont(QFont("Microsoft YaHei", 14 if self.mode == "word" else 16))
            ...
            if self.mode == "word":
                ...
                hsk_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                hsk_item.setFont(QFont("Microsoft YaHei", 9))
                self.table.setItem(row, 4, hsk_item)
            else:
                hsk_item = QTableWidgetItem(self._display_hsk_level(item))
                hsk_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                hsk_item.setFont(QFont("Microsoft YaHei", 9))
                self.table.setItem(row, 3, hsk_item)

    def _filter_table(self, text):
        text = text.strip()
        for row in range(self.table.rowCount()):
            value_item = self.table.item(row, 1)
            if not text or text in (value_item.text() if value_item else ""):
    ...
    def _export_csv(self):
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            ...
            if self.mode == "word":
                ...
                for rank, (word, count) in enumerate(self.word_stats["frequency"].items(), 1):
                    writer.writerow([rank, word, count, len(word), self._display_hsk_level(word)])
            else:
                writer.writerow(["Rank", "Character", "Count", "HSK Level"])
                for rank, (char, count) in enumerate(self.character_stats["frequency"].items(), 1):
                    writer.writerow([rank, char, count, self._display_hsk_level(char)])
